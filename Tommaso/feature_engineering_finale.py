...
def process_battle(battle_json, max_turns=30):
    ...
    **Output snapshot**:
    ...
    # --- MODIFICA (Punto 2): Rimossa feature 'combined_type_advantage' ---
    # Le feature '...seen_alive' e '...unseen_p2' vengono ora passate
    # separatamente al modello GBDT, che ne deciderà il peso.
...
def analyze_feature_redundancy(df: pd.DataFrame):
    """
    MODIFICA (Punto 3): Analisi Ridondanza Feature.
    
    Stampa una matrice di correlazione per le feature di danno "diff_".
    Questo aiuta a identificare quali feature sono ridondanti (es. corr > 0.95)
    e possono essere rimosse per sfoltire il modello.
    
    Le feature "Top 10" sono molto correlate. Vogliamo verificare se 
    'diff_ema_damage_dealt', 'diff_phys_damage', 'diff_spec_damage'
    siano solo variazioni di 'diff_damage_dealt'.
    """
    print("\n" + "="*80)
    print("MODIFICA (Punto 3): Analisi di Correlazione della Ridondanza")
    print("="*80)
    
    # Lista delle feature di danno differenziali potenzialmente ridondanti
    damage_features = [
        'diff_damage_dealt',
        'diff_ema_damage_dealt',
        'diff_phys_damage',
        'diff_spec_damage',
        'diff_damage_tempo',
        'diff_max_burst_3turn',
        'diff_ko_efficiency',
        'diff_fainted_pokemon' # Aggiunto per contesto
    ]
    
    # Filtra le colonne che esistono effettivamente nel DataFrame
    existing_features = [col for col in damage_features if col in df.columns]
    
    if not existing_features:
        print("WARN: Nessuna delle feature di danno differenziali trovata per l'analisi.")
        return

    print("Analisi delle seguenti feature (sub-set della Top 10):")
    print(existing_features)
    
    # Calcola e stampa la matrice di correlazione
    corr_matrix = df[existing_features].corr()
    
    print("\nMatrice di Correlazione (valori > 0.8 sono probabili ridondanze):\n")
    print(corr_matrix.to_string(float_format="%.3f"))
    print("\n" + "="*80)
    print("Azione consigliata: Se 'diff_ema_damage_dealt' o 'diff_phys_damage'")
    print("hanno correlazione > 0.95 con 'diff_damage_dealt', considera di")
    print("rimuoverli dal training per sfoltire il modello.")
    print("="*80 + "\n")
...
def create_feature_df(file_path, max_turns=30):
    ...
    # --- MODIFICA (Punto 2): Disabilitate feature di interazione manuali ---
    # Le GBDT (CatBoost, LightGBM) sono già efficaci nel trovarle da sole.
    # Lasciamo la funzione definita nel file, ma non la chiamiamo.
    # df = add_interaction_features(df)
    
    # --- MODIFICA (Punto 3): Aggiunta analisi di ridondanza ---
    # Stampa la correlazione tra le feature di danno per aiutare
    # nello sfoltimento manuale delle feature (feature selection).
